...
![](./images/upload.png){width = 400px}

Rather than using the bulk upload option, reporting entities can also do manual entry through the graphic user interface. For more information on the fields they fill out here, they can reference the Submission Data Mapping Document on the [resources page](https://www.cms.gov/OpenPayments/Resources/Reporting-Entities). Some of the information they must enter is as follows.

![](./images/manual1.png){width = 400px}
...
If the drug or biological name and NDC combination entered is valid, then a message will appear that asserts this validity. Otherwise, they will receive the following error message. They can correct this by going to the [NDC database](https://www.accessdata.fda.gov/scripts/cder/ndc/index.cfm) to obtain the correct combination of name and code. 
...
variable_info <- read_csv("./data_files/PY 2016 and upcoming years Submission Data Mapping Document.csv")
...
Now that we have looked at transfers of value across all specialties together, we can get a better understanding of how payments differ by Primary Specialty. 

<div style = "overflow-x: auto; height: 600px; width = 550px;">


```{r, total amounts per primary specialty, fig.width = 10}
...
It is clear that Allopathic & Osteopathic Physicians make up the vast majority of dollars paid, which is logical given that this division contains the highest number of subspecialties. Now, we can look at how payments are split up among the Allopathic & Osteopathic Physician specialties. 
...
# total payments by nature of payment for Internal Medicine
highest_paid_specialties %>% 
...
# plot median payment per provider by state
values_geo %>% 
